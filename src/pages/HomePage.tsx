...
import { MONTHS, CO2_COEFFICIENT } from '../data/mockData';
...
export default function HomePage({ year, t }: Props) {
  ...
  const elecData = data.electricity[selectedYear] ?? data.electricity[2023];
  const coeff = data.co2Coefficient ?? CO2_COEFFICIENT;
  const co2Monthly = elecData.map(v => Math.round(v * coeff));
  const kpi = data.kpi;
  ...
  }, [year, JSON.stringify(data.electricity), data.co2Coefficient]);
